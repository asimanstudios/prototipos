...
import type { ModAPruebaGroup, ModAPrueba, Suggestion, UserRole } from '@/lib/types';
import { Accordion, AccordionContent, AccordionItem, AccordionTrigger } from '@/components/ui/accordion';
import { Button, buttonVariants } from '@/components/ui/button';
import { Card, CardContent, CardHeader } from '@/components/ui/card';
import { ModsAPruebaTable } from './mods-a-prueba-table';
...
function SuggestionsSection({ 
    ...
    const canSuggest = useMemo(() => !['Miembro'].includes(role), [role]);
                            ...
                            <Select value={newSuggestion.modId} onValueChange={(v) => setNewSuggestion({...newSuggestion, modId: v})}>
                                ...
                                <SelectContent>
                                    {suggestibleMods.map(m => (
                                        <SelectItem key={m.id} value={m.id} disabled={(newSuggestion.type === 'promotion' || newSuggestion.type === 'warning') && m.resumen === 'Nuevo'}>{m.nombre}</SelectItem>
                                    ))}
                                </SelectContent>
                            </Select>
                        </div>
...
export function ModsAPruebaGroups() {
  const { role } = useUserRole();
  const { toast } = useToast();
  const [groups, setGroups] = useState<ModAPruebaGroup[]>([]);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const sanitizedGroups = (data.modsAPruebaGroups || []).map((g: ModAPruebaGroup) => {
          const sortedMods = [...(g.mods || [])].sort((a, b) => {
              if (a.resumen === 'Nuevo' && b.resumen !== 'Nuevo') return 1;
              if (a.resumen !== 'Nuevo' && b.resumen === 'Nuevo') return -1;
      ...
      toast({ title: 'Error', description: 'No se pudieron cargar los grupos de mods a prueba.', variant: 'destructive' });
    }
  }, [toast]);
  ...
  const persistChanges = useCallback(async (newGroups: ModAPruebaGroup[]) => {
    try {
      const response = await fetch('/api/data', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ modsAPruebaGroups: newGroups }),
      });
  ...
  const handleAddGroup = () => {
    ...
    const newGroup: ModAPruebaGroup = {
      id: newGroupId,
      nombre: `Nuevo Grupo`,
      revisadoPor: '',
      modsElites: '',
  ...
  const handleGroupChange = (groupId: string, field: keyof ModAPruebaGroup, value: any) => {
    const newGroups = groups.map(g => g.id === groupId ? { ...g, [field]: value } : g);
    setGroups(newGroups);
  };
  
  const handleModsChange = (groupId: string, mods: ModAPruebaGroup['mods']) => {
                ...
                <AccordionItem value={group.id} className="border-b-0">
                  <CardHeader className="p-4">
                    <div className="flex justify-between items-center">
                        {isEditing ? (
                            <Input value={group.periodo} onChange={(e) => handleGroupChange(group.id, 'periodo', e.target.value)} className="text-2xl font-bold border-2 border-primary" />
                        ) : (
                            <h2 className="text-2xl font-bold" style={{color: '#fbbf24'}}>{group.periodo}</h2>
                        )}
                      <div className="flex items-center gap-2">
                        {isEditing ? (
                          <>
                            <Button onClick={() => saveEditing(group.id)} size="sm" className="bg-green-600 hover:bg-green-700"><Save className="mr-2 h-4 w-4"/> Guardar</Button>
                          </>
                        ) : (
                          <>
                  ...
                  <AccordionContent>
                    <CardContent className="p-4 pt-0">
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-4 p-4 rounded-lg bg-black/30 border border-white/10">
                            {Object.entries({revisadoPor: "Revisado por", modsElites: "Mod Elites", senior: "Senior", directores: "Directores", nombre: "Nombre Grupo", hora: "Hora"}).map(([key, label]) => (
                                <div key={key}>
                                    <label className="text-xs font-bold text-muted-foreground">{label}</label>
                                    <Input 
                                        value={(group as any)[key]} 
                                        onChange={(e) => handleGroupChange(group.id, key as keyof ModAPruebaGroup, e.target.value)} 
                                        disabled={!isEditing} 
