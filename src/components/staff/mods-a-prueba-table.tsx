...
import type { ModAPrueba } from '@/lib/types';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Button, buttonVariants } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Trash2, PlusCircle, Edit, Save, User, ArrowUp, ArrowDown } from 'lucide-react';
...
type ModsAPruebaTableProps = {
  mods: ModAPrueba[];
  onModsChange: (mods: ModAPrueba[]) => void;
  canEdit: boolean;
  canDelete: boolean;
};

const resumenColors: { [key: string]: string } = {
...
export function ModsAPruebaTable({ mods, onModsChange, canEdit, canDelete }: ModsAPruebaTableProps) {
  const [editingRow, setEditingRow] = useState<string | null>(null);
  const [newRowData, setNewRowData] = useState<Partial<ModAPrueba>>({});
  const [viewingMod, setViewingMod] = useState<ModAPrueba | null>(null);

  const handleAddNew = () => {
    const newMod: ModAPrueba = {
      id: `new-${Date.now()}`,
      nombre: 'Nuevo Mod',
      entrenos: null,
  ...
  const handleUpdate = (id: string, field: keyof ModAPrueba, value: any) => {
    onModsChange(mods.map(mod => mod.id === id ? { ...mod, [field]: value } : mod));
  };
  
  const handleNumericUpdate = (id: string, field: keyof ModAPrueba, value: string) => {
  ...
  const handleViewMod = (mod: ModAPrueba) => {
  ...
  const moveRow = (index: number, direction: 'up' | 'down') => {
    if (
      ...
      (direction === 'down' && index === mods.length - 1)
    ) {
      return;
    }
    const newMods = [...mods];
      ...
      <Table className="min-w-full divide-y divide-border bg-card/80 text-xs">
        <TableHeader>
          <TableRow className="hover:bg-transparent">
            {['Nombre', 'SGT+', 'Entrenos', 'Entrenos Propios', 'Trys', 'Roles de PJ', 'Rol Espontaneo', 'Misiones', 'Inactividad', 'Resumen', 'Servidor', 'Acciones'].map(header => (
        ...
        <TableBody className="divide-y divide-border">
          {mods.map((mod, index) => {
            ...
            return (
              <TableRow key={mod.id} className="hover:bg-primary/10">
                <TableCell 
                    className={cn("p-1 align-middle flex items-center gap-1", { 'cursor-pointer hover:underline': !isEditing }, "min-w-[150px]")}
                    onClick={() => handleViewMod(mod)}
                >
                    <Input type="text" value={mod.nombre} onChange={e => handleUpdate(mod.id, 'nombre', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7"/>
                ...
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.entrenos ?? ''} onChange={e => handleNumericUpdate(mod.id, 'entrenos', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" /></TableCell>
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.entrenosPropios ?? ''} onChange={e => handleNumericUpdate(mod.id, 'entrenosPropios', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" /></TableCell>
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.trys ?? ''} onChange={e => handleNumericUpdate(mod.id, 'trys', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" /></TableCell>
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.rolesPJ ?? ''} onChange={e => handleNumericUpdate(mod.id, 'rolesPJ', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" style={{backgroundColor: 'hsl(var(--muted))'}}/></TableCell>
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.rolEspontaneo ?? ''} onChange={e => handleNumericUpdate(mod.id, 'rolEspontaneo', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" style={{backgroundColor: 'hsl(var(--muted))'}}/></TableCell>
                <TableCell className="p-1 align-middle"><Input type="number" value={mod.misiones ?? ''} onChange={e => handleNumericUpdate(mod.id, 'misiones', e.target.value)} disabled={!isEditing} className="bg-transparent border-0 text-left text-xs h-7 w-16" style={{backgroundColor: 'hsl(var(--muted))'}}/></TableCell>
        ...
        <Dialog open={!!viewingMod} onOpenChange={(open) => !open && setViewingMod(null)}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Detalles del registro del moderador a prueba.
                    </DialogDescription>
                </DialogHeader>
                <Card>
                    <CardContent className="p-4 grid grid-cols-2 gap-4 text-sm">
                        ...
                        <div className="font-semibold">Resumen:</div><div>{viewingMod.resumen && <Badge className={cn(resumenColors[viewingMod.resumen])}>{viewingMod.resumen}</Badge>}</div>
                    </CardContent>
                </Card>
            </DialogContent>
        </Dialog>
